...
export class ClientsService {
  ...
  async findAll(): Promise<Client[]> {
    const result = await this.pool.request().query('SELECT * FROM Clients');
  ...
  async update(id: number, dto: UpdateClientDto): Promise<void> {
    this.logger.warn(
      `Called update() with id ${id} dto =${JSON.stringify(dto)} `,
    );
  ...
  async remove(id: number): Promise<void> {
    await this.pool.request().input('id', id).query(`
